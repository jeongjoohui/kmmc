...
<div class="webzLat">
    <h2 class="webzLatTit"><a href="<?php echo G5_BBS_URL ?>/board.php?bo_table=<?php echo $bo_table ?>"><?php echo $bo_subject ?> <u></u></a></h2>
    <ul>
    <?php for ($i=0; $i<count($list); $i++) { 
		...
		if($thumb['src']) {
			...
			if($youtubeID[2] == "youtu.be") {
				$img_content = '<i class="webzLatThumb" style="background-image:URL(https://img.youtube.com/vi/'.$youtubeID[3].'/sddefault.jpg" alt="'.$list[$i]['subject'].');"><img src="'.G5_THEME_URL.'/plugin/websiting/imageSpacer.php?w='.$thumb_width.'&h='.$thumb_height.'" class="youtubeImbBlank" alt=""></i>';
			} else {
				$img_content = "<!-- 이미지 없음 -->";
			}
		}
