...
add_stylesheet('<link rel="stylesheet" href="'.$new_skin_url.'/style.css?v2">', 0);
?>
...
<fieldset id="new_sch">
    ...
    <input type="text" name="mb_id" value="<?php echo $mb_id ?>" id="mb_id" required class="frm_input" size="40" placeholder="회원 아이디만 검색 가능">
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <?php } ?>
        <td class="mvSetSubject">
			<div><a href="./new.php?gr_id=<?php echo $list[$i]['gr_id'] ?>"><?php echo $gr_subject ?></a> <em class="fa fa-angle-right"></em> <a href="./board.php?bo_table=<?php echo $list[$i]['bo_table'] ?>"><?php echo $bo_subject ?></a></div>
			<a href="<?php echo $list[$i]['href'] ?>" class="new_tit"><?php echo $list[$i]['comment'] ?><?php echo $wr_subject ?></a>
		</td>
        <td class="td_name"><div><?php echo $list[$i]['name'] ?></div> <div><?php echo $list[$i]['datetime2'] ?></div></td>
    </tr>
    <?php }  ?>
